...
同步时按「文档类型」筛选 EMA 数据源。类型清单与文档数按数据源实际情况自动统计（当前约 86 种），以「开始同步」对话框与「设置」页实时展示的为准（两者共用同一份清单），常用类型如下：

| 类型标识 | 界面名称 | 内容 |
| :--- | :--- | :--- |
| `scientific-guideline` | 科学指南 | 研发 / 质量 / 临床 / 安全性各领域指南（反思文件、概念文件现亦归入此类型） |
| `regulatory-procedural-guideline` | 监管程序指南 | 上市许可、变更、审评等监管程序性规定 |
| `medicine-qa` | 问答文件 | 针对具体药品与安全问题的官方 Q&A（旧标识 `q-and-a` 已停用） |
| `assessment-report` | 评估报告 | 药品欧洲公共评估报告（EPAR）的评估正文 |
| `scientific-conclusion` | 科学结论 | EPAR 的科学结论与依据章节 |
| `product-information` | 产品信息 | SmPC 产品特性概要与包装说明书 |
| `smop` / `smop-initial` | 意见摘要 | 委员会上市许可意见的公开摘要（上市后 / 初次） |
| `procedural-steps-after` | 上市后程序步骤 | 获批后采取的程序步骤与科学信息 |
| `psusa` | 定期安全评估 | 各国上市药品的定期安全性更新单一评估 |
| `rmp` | 风险管理计划 | 风险管理计划及其摘要 |
| `pip-decision` | 儿科研究计划 | PIP 批准 / 修改决定（数据源中数量最多） |
| `orphan-designation` | 孤儿药认定 | 孤儿药认定公开摘要 |
| `herbal-monograph` | 草药专论 | HMPC 欧盟草药专论 |
| `mrl-report` | 残留限量报告 | 兽药活性成分最大残留限量（MRL）评估 |
| `public-statement` | 公开声明 | 撤市等原因的官方声明 |
| `referral` | 转介程序 | 欧盟层面针对安全问题 / 分歧的统一审查 |
| `dhpc` | 专业人员安全通报 | 直接发给医疗专业人员的药物警戒警示 |

其余类型（会议议程 / 纪要 / 幻灯、委员会报告、新闻稿、草药系列、EU 以外获批信息等）同样在对话框中按文档数降序列出，勾选即同步。
...
「开始同步」支持在对话框中按次覆盖参数：文档类型、官方状态、关键词、更新起始日期、限制数量、并发数、代理，并可勾选 **Dry-run 预览模式**。「设置」页与同步对话框使用同一份类型清单——按 EMA 数据源中实际存在的类型自动生成并标注各类型的文档数，对话框默认勾选「设置」页中的默认类型，留空提交即使用默认。
